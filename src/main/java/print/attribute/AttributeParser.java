...
import print.attribute.ipp.jobdesc.LdJobStateReason;
import print.exception.EndOfAttributesException;
	...
	*/
	...
	private static Object[] parseValues(int valueTag, InputStream in) throws IOException {
		...
		if (valueTag == IppValueTag.INTEGER.getValue() || valueTag == IppValueTag.ENUM.getValue()) {
			Integer number = Integer.valueOf(parseInt4(in));
			...
			Integer bool = Integer.valueOf(in.read());
			values = new Object[] { bool };
		} else if (valueTag == IppValueTag.RANGE.getValue()) {
			Integer lowerBound = Integer.valueOf(parseInt4(in));
			Integer upperBound = Integer.valueOf(parseInt4(in));
